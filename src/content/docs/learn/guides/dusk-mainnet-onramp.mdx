...
**Recommendation:** It is recommended to perform both a **deposit** and **stake** transaction if you want to stake at genesis. If you do not want to stake, you can skip step 4. If you only stake, you will not have funds available for essential network transactions like reward withdrawals or transfers, we therefore recommend to do step 3 as well. It is still possible to migrate DUSK through our [mainnet migration bridge](/learn/guides/mainnet-migration) afterwards. For example scenarios, see [this](#can-you-describe-a-couple-scenarios) FAQ question. Keep in mind that to perform step 4, you do not have to do step 3.
...
Use the `deposit` function to on-ramp DUSK tokens for inclusion in the Dusk genesis state. If you want to stake, do not deposit your full balance. Instead, check step 4. If you only want to deposit, you can skip step 4. Read [Deposit versus Staking](#depositing-versus-staking) for more details.
...
Use the `stake` function if you want to participate in genesis staking. Before doing the stake call, make sure you deposited a small amount of funds to do staking operations later. Read [Deposit versus Staking](#depositing-versus-staking) for more details.
